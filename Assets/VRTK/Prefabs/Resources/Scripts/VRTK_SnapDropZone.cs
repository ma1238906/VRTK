    ...
    public class VRTK_SnapDropZone : MonoBehaviour
    {
        ...
        protected virtual void CreateHighlightersInEditor()
        {
            ...
            if (VRTK_SharedMethods.IsEditTime())
            {
                ...
                {
                    VRTK_Logger.Warn(VRTK_Logger.GetCommonMessage("REQUIRED_COMPONENT_MISSING_FROM_GAMEOBJECT", new string[] { "SnapDropZone:" + name, "Joint", "the same", " because the `Snap Type` is set to `Use Joint`" }));
                }
        ...
        protected virtual void SetSnapDropZoneJoint(Rigidbody snapTo)
        {
            ...
            {
                VRTK_Logger.Error(VRTK_Logger.GetCommonMessage("REQUIRED_COMPONENT_MISSING_FROM_GAMEOBJECT", new string[] { "SnapDropZone:" + name, "Joint", "the same", " because the `Snap Type` is set to `Use Joint`" }));
                return;
            }
            if (snapTo == null)
            {
                VRTK_Logger.Error(VRTK_Logger.GetCommonMessage("REQUIRED_COMPONENT_MISSING_FROM_GAMEOBJECT", new string[] { "VRTK_SnapDropZone", "Rigidbody", "the `VRTK_InteractableObject`" }));
                return;
            }
