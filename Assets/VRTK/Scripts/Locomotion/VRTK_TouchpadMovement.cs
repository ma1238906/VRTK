...
namespace VRTK
{
    ...
    public class VRTK_TouchpadMovement : MonoBehaviour
    {
        ...
        protected virtual void Awake()
        {
            ...
            if (!playArea)
            {
                VRTK_Logger.Error(VRTK_Logger.GetCommonMessage("SDK_OBJECT_NOT_FOUND", new string[] { "PlayArea", "Boundaries SDK" }));
            }

            headset = VRTK_DeviceFinder.HeadsetTransform();
            if (!headset)
            {
                VRTK_Logger.Error(VRTK_Logger.GetCommonMessage("SDK_OBJECT_NOT_FOUND", new string[] { "HeadsetTransform", "Headset SDK" }));
            }
        ...
        protected virtual void Start()
        {
            ...
            if (!bodyCollider)
            {
                VRTK_Logger.Error(VRTK_Logger.GetCommonMessage("REQUIRED_COMPONENT_MISSING_FROM_GAMEOBJECT", new string[] { "VRTK_TouchpadMovement", "CapsuleCollider", "the PlayArea" }));
            }
        }
