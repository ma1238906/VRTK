...
namespace VRTK
{
    ...
    public class VRTK_ControllerHighlighter : MonoBehaviour
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            if (controllerAlias == null)
            {
                VRTK_Logger.Error(VRTK_Logger.GetCommonMessage("REQUIRED_COMPONENT_MISSING_NOT_INJECTED", new string[] { "VRTK_ControllerHighlighter", "Controller Alias GameObject", "controllerAlias", "the same" }));
                return;
            }
        ...
        protected virtual Transform GetElementTransform(string path)
        {
            ...
            if (!cachedElements.ContainsKey(path) || cachedElements[path] == null)
            {
                if (!modelContainer)
                {
                    VRTK_Logger.Error(VRTK_Logger.GetCommonMessage("SDK_OBJECT_NOT_FOUND", new string[] { "Controller Model", "Controller SDK" }));
                    return null;
                }
