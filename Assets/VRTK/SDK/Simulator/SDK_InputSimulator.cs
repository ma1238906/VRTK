    ...
    public class SDK_InputSimulator : MonoBehaviour
    {
        ...
        public static GameObject FindInScene()
        {
            if (cachedCameraRig == null && !destroyed)
            {
                ...
                if (!cachedCameraRig)
                {
                    VRTK_Logger.Error(VRTK_Logger.GetCommonMessage("REQUIRED_COMPONENT_MISSING_FROM_SCENE", new string[] { "VRSimulatorCameraRig", "SDK_InputSimulator", ". check that the `VRTK/Prefabs/VRSimulatorCameraRig` prefab been added to the scene." }));
                }
            }
